...
[^fn3]: The first use of the term 'rational expectations' that I know is by Hurwicz (1946, p. 133), who did not define the term, but used it in discussing ways of modelling the behavior of a single firm facing an unknown distribution of future output prices. Muth's {cite:year}`Muth1961` definition emphasizes the second aspect of the concept, consistency of perceptions. {cite:t}`Hurwicz1951` used both elements of the concept of rational expectations, though he did not use the term in his discussion of econometric policy evaluation procedures.
...
[^fn5]: The example in this section is close to that of {cite:t}`LucasPrescott1971`.
...
[^fn7]: This is a version of the demand function for money used by Phillip {cite:t}`Cagan1956` to study hyperinflations. The objective function that we have attributed to the household corresponds to one that is often used in versions of Paul Samuelson's {cite:year}`Samuelson1958` overlapping generations model; for example, see Neil {cite:t}`Wallace1980`.
...
[^fn10]: Versions of this result are due to Russell {cite:t}`Boyer1971` and {cite:t}`KarekenWallace1981`.
...
In the methodology of positive economics of Milton {cite:t}`FriedmanM1953`, economic models are judged by how the outcomes that they predict match data measuring the economy. From the standpoint of this methodology, the behavioral emptiness of rational expectations is neither virtue nor defect. To justify the research effort called forth by the bounded rationality program, we require some prospective payoffs in terms of concrete questions that rational expectations models are not likely to be capable of answering. What are these questions, and where are the likely payoffs?[^fn24],[^fn25]

[^fn24]: Niels Bohr said, 'It is wrong to think that the task of physics is to find out how nature is. Physics concerns what we can say about nature.' This quotation is from the biography of Bohr by Abraham {cite:t}`Pais1992`.
...
[^fn26]: David {cite:t}`Kreps1990` discusses issues involving equilibrium selection in games. Many of the issues in this essay, and some of the viewpoints, are identical to ones discussed by Kreps in related contexts.
...
[^fn28]: The description in the text fits my own work on the ends of big hyperinflations (Sargent 1986, ch. 3), but misses some influential analyses of hyperinflations. {cite:t}`FloodGarber1983` and {cite:t}`LaHaye1985` have focused on the effects during the hyperinflation of speculation about when a stabilization will occur. Also, see Hamilton's {cite:year}`Hamilton1989` model of regime switches.
...
[^fn31]: See {cite:t}`BeersSargentWallace1983`.
...
[^fn32]: Thus, the banks are assumed to understand the workings of a 'Modigliani-Miller theorem,' which describes the incentives that the system seems to have given them for increasing the risk in their portfolios to the benefit of their shareholders. See {cite:t}`KarekenWallace1978` and {cite:t}`Merton1978`. But the banks, investors, and creators of the Federal Reserve are assumed not to foresee (or care about?) the system-wide consequences of their behaviors.
...
[^fn33]: See {cite:t}`SargentWallace1982` and Bruce {cite:t}`SmithB1988` for comparisons of the quantity theory and real bills regimes.
