...
[^fn73]: {cite:t}`MarcetSargent1989a` show that the limiting behavior of $\beta$ is governed by the associated differential equation $(d/dt)\beta = a + (b - 1)\beta$, which is stable for $b < 1$. The right hand side of this differential equation can be expressed as $T(\beta) - \beta$, where $T(\beta)$ is the mapping from the *perceived* forecast of prices $\beta$ to the *optimal* forecast of prices $a + b\beta$. Stephen {cite:t}`DeCanio1979` and George Evans (1985, 1989) used the operator $T(\beta)$ to define a notion of expectational stability. {cite:t}`MarcetSargent1989a` described a sense in which the operator $T(\beta) - \beta$ governs the convergence of least squares learning schemes in a class of models.

[^fn74]: Jasmina {cite:t}`Arifovic1991` has studied a version of Bray's model in which Bray's representative least squares learner is replaced by a population of heterogeneous agents with heterogeneous beliefs. She applied a genetic algorithm to this environment, and found that the population can sometimes learn its way to a rational expectations equilibrium even when Bray's necessary condition $b < 1$ is violated.
...
[^fn75]: See Mark {cite:t}`Feldman1987` for a study of the convergence of a model with a collection of Bayesian agents who start out with divergent priors. Also, see {cite:t}`ElGamalSundaram1993`.
...
[^fn76]: {cite:t}`MarcetSargent1989b` describe systems with heterogeneous beliefs in which heterogeneity remains in the rational expectations equilibrium because people are assumed to be differentially informed.
...
[^fn78]: See {cite:t}`CassShell1983`, {cite:t}`Azariadis1981`, and {cite:t}`AzariadisGuesnerie1986`.
...
[^fn81]: See {cite:t}`EvansHonkapohja1993b` for a discussion of some of the features of constant gain algorithms.
...
[^fn82]: See {cite:t}`MarcetMarshall1992` and {cite:t}`Sargent1991`. Also, see Kenneth Judd (1990, 1992) for descriptions of a variety of numerical methods for computing approximate equilibria.
...
[^fn84]: See Kenneth {cite:t}`Judd1992` for a critical survey of and guide to such methods.
...
[^fn85]: See {cite:t}`MarcetMarshall1992` for a formal analysis of the algorithm.
...
[^fn87]: The environment is the one studied by {cite:t}`KarekenWallace1981`.
...
The environment is one analyzed in detail by John {cite:t}`Hussman1992`.[^fn95] There is a competitive market for a stock that is a claim on a dividend process $\{d_t\}$ governed by

[^fn95]: Hussman's work is related to work by Jiang {cite:t}`Wang1990`. For other work on methods of circumventing the no trade theorem, see Harald {cite:t}`Uhlig1992`.
...
[^fn100]: The work described in this section is from {cite:t}`HussmanSargent1993`.
...
[^fn114]: See {cite:t}`MarcetSargent1992` for an analysis of rates of convergence in a particular model, with part of the analysis being based on the theorems of Benveniste et al., and another part being based on Monte Carlo methods. Also see {cite:t}`LjungPflugWalk1992`.

[^fn115]: See Chung-Ming {cite:t}`Kuan1989` and {cite:t}`Mohr1990` for useful early contributions. {cite:t}`MarcetSargent1993` state a proposition about a rate of convergence.
