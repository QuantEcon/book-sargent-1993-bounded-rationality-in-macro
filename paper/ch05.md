...
Margaret {cite:t}`Bray1982` studied a model that exhibits several features of systems that are adapting their way to a rational expectations equilibrium. These features include:
...
Given an initial condition for $\beta$, equations {eq}`eq-5-2` and {eq}`eq-5-3` determine the evolution of $(p, \beta)$ through time, where $\beta_t$ is interpreted as people's expectation of what $p_{t+1}$ will be.[^fn73] Bray studied the circumstances under which $\beta_t$ and the distribution of $p_{t+1}$, which evolve interdependently, would converge to a rational expectations equilibrium. That is, she studied the conditions under which $\beta_t$ would converge to the value $\beta = a/(1 - b)$.

[^fn73]: This model has the special feature that, in the rational expectations equilibrium, the unconditional expectation equals the conditional expectation, a consequence of there being no time-varying state variables in the rational expectations equilibrium.
...
Margaret Bray showed that, if $b < 1$, system {eq}`eq-5-2`, {eq}`eq-5-3` will converge to a rational expectations equilibrium with probability one. She also noted that $b < 1$ is a necessary condition for convergence to a rational expectations equilibrium.[^fn74] Furthermore, she showed that, if the system does not converge to a rational expectations equilibrium, it does not converge at all.[^fn75]

[^fn74]: {cite:t}`MarcetSargent1989a` show that the limiting behavior of $\beta$ is governed by the associated differential equation $(d/dt)\beta = a + (b - 1)\beta$, which is stable for $b < 1$. The right hand side of this differential equation can be expressed as $T(\beta) - \beta$, where $T(\beta)$ is the mapping from the *perceived* forecast of prices $\beta$ to the *optimal* forecast of prices $a + b\beta$. Stephen {cite:t}`DeCanio1979` and George Evans (1985, 1989) used the operator $T(\beta)$ to define a notion of expectational stability. {cite:t}`MarcetSargent1989a` described a sense in which the operator $T(\beta) - \beta$ governs the convergence of least squares learning schemes in a class of models.

[^fn75]: Jasmina {cite:t}`Arifovic1991` has studied a version of Bray's model in which Bray's representative least squares learner is replaced by a population of heterogeneous agents with heterogeneous beliefs. She applied a genetic algorithm to this environment, and found that the population can sometimes learn its way to a rational expectations equilibrium even when Bray's necessary condition $b < 1$ is violated.
...
Why not repair the irrationality indicated by the discrepancy between $\beta_{t-1}$ and $E p_t | z_t^*$ by going back to the original model and replacing $\beta_{t-1}$ by $E p_t | z_t^*$? Evidently, using this new theory of price expectations would require us to modify the actual law of motion for prices, which would render our new scheme suboptimal again. But we can use this new actual law of motion as our theory of expectations. This line starts us on a recursion, which has been taken up by {cite:t}`BrayKreps1987`, who show that in the limit it leads us back to a rational expectations in which agents are learning 'within' the equilibrium, but not 'about' the equilibrium. Bray and Kreps argue against following this recursion to its limit if it is 'bounded rationality' that we are after.[^fn76]

[^fn76]: See Mark {cite:t}`Feldman1987` for a study of the convergence of a model with a collection of Bayesian agents who start out with divergent priors. Also, see {cite:t}`ElGamalSundaram1993`.
...
[^fn77]: {cite:t}`MarcetSargent1989b` describe systems with heterogeneous beliefs in which heterogeneity remains in the rational expectations equilibrium because people are assumed to be differentially informed.
...
[^fn82]: See {cite:t}`EvansHonkapohja1993b` for a discussion of some of the features of constant gain algorithms.
...
[^fn83]: See {cite:t}`MarcetMarshall1992` and {cite:t}`Sargent1991`. Also, see Kenneth Judd (1990, 1992) for descriptions of a variety of numerical methods for computing approximate equilibria.

(sec-learning-hard-way)=
### Learning the hard way

In the preceding model, learning occurs between *non-overlapping* generations of grandparents to grandchildren, with the grandchildren adjusting their grandparents' saving choice after observing the consequences of their grandparents' choice.[^fn84] This model requires little of agents in the way of 'theorizing,' at the cost of rendering their learning dependent on the behaviors and experiences of their predecessors.

[^fn84]: A byproduct of setting things up in this way is the alternation of turns between odd and even sets of agents. This 'two-population' feature of the learning algorithm duplicates or resembles the experimental environments of {cite:t}`MarimonSunder1992` and {cite:t}`Arifovic1993`, to be discussed in the next chapter.
...
There exist a number of methods for solving a functional equation like {eq}`eq-5-13` numerically. All of these methods replace the function $f(G)$ with a finite-parameter approximation $f(G, \theta)$, then find values of the parameters $\theta$ that come as close as possible to satisfying {eq}`eq-5-13`.[^fn85]

[^fn85]: See Kenneth {cite:t}`Judd1992` for a critical survey of and guide to such methods.

(sec-parameterized-expectations)=
### Method of parameterized expectations

Here is how Albert Marcet's method of parameterized expectations can be used approximately to solve the functional equation {eq}`eq-5-13`. For convenience, write the right side of {eq}`eq-5-13` as $E_t k(G_t, G_{t+1})$ where $k(G_t, G_{t+1}) = u'(w_2 + (Nf(G_{t+1}) - G_{t+1})/N)(Nf(G_{t+1}) - G_{t+1})/Nf(G_{t+1})$.[^fn86]

[^fn86]: See {cite:t}`MarcetMarshall1992` for a formal analysis of the algorithm.
...
For the present example, we could let $x_t = [R_t, G_t]'$. At time $t$, we would let behavior be determined by the solution of a version of {eq}`eq-5-8b` in which the conditional expectation on the right side is evaluated with respect to the conditional distribution for $R_t$ conditional on $G_t$ that can be deduced from the joint density $\hat{F}_{t-1}(x)$.[^fn87]

[^fn87]: Chen and White (1992, 1993) have attained results on rates of convergence of such nonparametric estimators under assumptions permitting less feedback from agents' behavior to outcomes than the present example admits.

(sec-learning-exchange-rate)=
## Learning in a model of the exchange rate

We now study an environment for which the rational expectations equilibrium exchange rate is indeterminate, but we expel all rational agents and replace them with adaptive agents.[^fn88] We endow these adaptive agents with learning algorithms and initial values for their decisions, which serve to render the exchange rate and all other endogenous variables determinate.[^fn89] We want to study how the exchange rate behaves, and whether a ghost of indeterminacy still lurks.

[^fn88]: The environment is the one studied by {cite:t}`KarekenWallace1981`.

[^fn89]: See {cite:t}`Calvo1988` and {cite:t}`EvansHonkapohjaSargent1993` for setups in which a fraction $1 - \mu$ of agents is rational, and a fraction $\mu$ is 'adaptive' in particular senses. Both of these contributions are concerned with studying how dynamics might differ from the rational expectations dynamics even with a very small $\mu$.
...
The environment is one analyzed in detail by John {cite:t}`Hussman1992`.[^fn97] There is a competitive market for a stock that is a claim on a dividend process $\{d_t\}$ governed by

[^fn97]: Hussman's work is related to work by Jiang {cite:t}`Wang1990`. For other work on methods of circumventing the no trade theorem, see Harald {cite:t}`Uhlig1992`.
...
Following Tirole, we assume that the asset is available in fixed supply $\bar{q}$, which for convenience we assume to be zero.[^fn98] A rational expectations equilibrium is a stochastic process for $\{p_t\}$ that satisfies the market-clearing condition

[^fn98]: The important thing is that the supply is fixed over time.
...
For ease of exposition, we shall assume that in forming conditional expectations, agents of both classes condition only on the most recent observation $z_{jt}$, which means that we set $I_{jt} = z_{jt}$. {cite:t}`Hussman1992` and {cite:t}`Sargent1991` describe how to set things up to condition on the (infinite) history of $z_{jt}$. We also replace conditional expectations with linear regressions, a step we can defend by one of two standard justifications.[^fn99] Substituting the demand functions {eq}`eq-5-18` into equilibrium condition {eq}`eq-5-19` gives

[^fn99]: Either assume that all distributions are multivariate normal or restrict decision rules to be linear.
...
Because we have assumed that the information vector $z_{at} \neq z_{bt}$, {eq}`eq-5-21` can hold $\forall t$ only if both sides are constant over time. However, {eq}`eq-5-20` with $E(y_t \mid z_{at}) = \alpha_0$, $\alpha_0$ a constant implies $E(y_t \mid z_{at}) = E(y_t \mid z_{bt}) = 0$.[^fn100] Substituting $E y_t | z_{jt}$ into the demand functions shows the no-trade outcome. The condition that $E y_t | I_{jt} = 0$ $\forall j$ shows that the market price adjusts to reveal fully all of the private information that is relevant for predicting excess returns.

[^fn100]: Applying the law of iterated expectations to {eq}`eq-5-20`, and noting that $E(E(y_t \mid z_{at})) = E(E(y_t \mid z_{bt})) = \alpha$, implies $(1 + \phi_b/\phi_a)\alpha = 0$, or $\alpha = 0$.
...
In order both to study the rational expectations equilibrium in more detail and to provide a framework from which we can expel rational agents and resettle adaptive agents, it is useful to have a way of computing the rational expectations equilibrium. We follow Hussman and adapt the apparatus of {cite:t}`MarcetSargent1989b` to this purpose. A trader of type $j$ observes the history of $z_{jt} = [p_t \ s_t^j \ d_t]'$, fits the vector autoregression,[^fn101]

[^fn101]: The vector $\zeta_{jt}$ is the innovation vector.
...
[^fn104]: The price differences show higher-than-normal kurtosis, which decreases toward three as the system converges to a rational expectations equilibrium.
...
{cite:t}`MarcetSargent1989a` describe a version of this model with adaptive agents. Firms formulate and recursively estimate an autoregression of the form {eq}`eq-5-30`, using the stochastic approximation algorithm
...
{cite:t}`MarcetSargent1989a` applied methods developed by Lennart {cite:t}`Ljung1977` to describe the sense in which the limiting behavior of the stochastic difference equations {eq}`eq-5-35`, {eq}`eq-5-36` is governed by the associated ordinary differential equations
...
I shall illustrate the kind of analysis that can be done with stochastic approximation methods in the context of a particular example, namely our model with a stochastic government deficit.[^fn109] Consider a version of that model in which agents are learning via a parametric model which they fit to the distribution of the return on currency. In particular, assume that agents generate forecasts of the rate of return on currency by fitting the parametric model

[^fn109]: This section is based on {cite:t}`MarcetSargent1989a` and {cite:t}`Woodford1990`. {cite:t}`BullardDuffy1993` study least squares learning in an economy with overlapping generations of $n$-period lived agents. For $n \geq 4$, they find that least squares learning *fails* to converge locally to a rational expectations equilibrium. Also see {cite:t}`Bullard1991` for a discussion of how complicated nonlinear dynamics can sometimes emerge out of least squares learning.
...
The model has the self-referential property that, when agents forecast according to the rule {eq}`eq-5-38` and when the $\theta$'s are updated according to {eq}`eq-5-39`, the optimal forecast has a form
...
where $E_t$ is the conditional expectation operator, $h(\cdot)$ is a function mapping $G_t$ into the least squares forecast $E_t R_t$, and $\{u_t\}$ is a random process with the property that $u_t$ is orthogonal to every (Borel measurable) function of $G_t$.[^fn110] The function $h(G)$, which depends on $(\theta_t, M_t)$, is determined implicitly by the process of solving the model.[^fn111]

[^fn110]: This property of $u_t$ identifies $h(\cdot)$ as the conditional expectation function.

[^fn111]: The function $h$ is understood to embed the dependence of $\theta_{t+1}$ on $\theta_t$, $M_t$ via equation {eq}`eq-5-39`. The price level $p_{t+1}$ depends on $\theta_{t+1}$ because $\theta_{t+1}$ influences savings behavior at $t + 1$.
...
Application of arguments in the spirit of {cite:t}`LjungSoderstrom1983` and {cite:t}`KushnerClark1978` can be used to show that the limiting behavior of the system of stochastic difference equations defined by the Markov process for $G$ and equations {eq}`eq-5-40` and {eq}`eq-5-41` is determined by an associated system of ordinary differential equations. This associated differential equation is derived by conducting the following mental experiment. Temporarily suspend the operation of system {eq}`eq-5-41`, and consider the system operating with a *fixed* $\theta$ for ever. Assume that *this* system converges to a unique invariant distribution,[^fn112] and let $h^*(G_t) = T(\theta)(G_t)$ be the conditional expectation of $R_t$ evaluated with respect to this invariant distribution. Form the associated differential equation system:

[^fn112]: i.e., a unique asymptotic stationary distribution.
...
(d) Suppose that the ordinary differential equation {eq}`eq-5-42` is globally stable about a unique rest point. Then there exists a modification of the recursive algorithm for $\theta_t$, $M_t$ which converges almost surely to the rest point.[^fn114]

(e) Convergence theorems require that $\{\gamma_t\}$ look like $\{1/t\}$. Convergence will not occur with 'constant-gain' versions of the algorithm.[^fn115]

(f) Few results are available on *rates* of convergence. However, theorems described by {cite:t}`BenvenisteMétivierPriouret1990` can sometimes be used to show that a necessary and sufficient condition for $\sqrt{T}$-convergence of $\theta_t$ to the fixed point is that the eigenvalues of the Jacobian of the linear approximation to the small o.d.e. {eq}`eq-5-43` at the fixed point are all less than $-1/2$ in modulus. Notice that this condition is stronger than the necessary condition for 'local stability' of the algorithm at the fixed point, namely that the eigenvalues of this same linear approximation are less than 0 in modulus.

[^fn114]: The modifications are devices that 'project' the estimator back into the intersection of the domain of attraction of the fixed point with the set of values of $\theta$ for which the system converges to an asymptotically stationary distribution for $R_t$, $G_t$. See {cite:t}`Ljung1977`, {cite:t}`LjungSoderstrom1983`, and {cite:t}`MarcetSargent1989a` for a discussion of various ways of modifying the algorithm. What is needed to get the stochastic approximation approach to yield almost sure convergence to a fixed point of the o.d.e. is some device that assures that the algorithm infinitely often visits the domain of attraction of the fixed point of the o.d.e.

[^fn115]: The most that can be hoped for with constant-gain versions of the algorithm is convergence in a stochastic sense of visiting a specified neighborhood of a fixed point of the o.d.e. with a relative frequency that depends, among other things, on the gain parameter $\gamma$.

Propositions like (a), (b), (c), and (e) are not difficult to obtain, and can be expected to apply across a wide variety of models, both linear models like those studied by Marcet and Sargent (1989a, 1989b, 1992) and nonlinear ones like the ones studied by {cite:t}`Woodford1990`. Propositions like (d) are harder to obtain, and often involve delicate and involved computations to verify assumptions sufficient to assure almost sure convergence. The amount of work to be done depends on the details of the device that is used to force the algorithm infinitely often into the domain of attraction of a fixed point. So far very little formal work has been done along the lines of proposition (f) about rates of convergence.[^fn116] [^fn117]

[^fn116]: See {cite:t}`MarcetSargent1992` for an analysis of rates of convergence in a particular model, with part of the analysis being based on the theorems of Benveniste et al., and another part being based on Monte Carlo methods. Also see {cite:t}`LjungPflugWalk1992`.

[^fn117]: See Chung-Ming {cite:t}`Kuan1989` and {cite:t}`Mohr1990` for useful early contributions. {cite:t}`MarcetSargent1993` state a proposition about a rate of convergence.

(sec-ch5-conclusions)=
## Conclusions

The examples in this chapter all take an environment that had been studied under rational expectations and add a source of transient dynamics coming from adaptive least squares learning. The dynamics are transient because the 'fundamentals' in these environments are time-invariant, and because the adaptive algorithms we have given our boundedly rational agents eventually settle upon good time-invariant decision rules for those environments.[^fn118]

[^fn118]: The exceptions are the constant-gain algorithms.
...
[^fn119]: Put differently, a regime that allows the exchange rate to be history-dependent seems to be an ill-formed mechanism.
